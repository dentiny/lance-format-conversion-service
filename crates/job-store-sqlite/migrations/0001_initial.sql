CREATE TABLE IF NOT EXISTS jobs (
    ...
    lease_owner TEXT,
    lease_expiration_timestamp INTEGER,
    ...
    CHECK (
        (status = 'running' AND lease_owner IS NOT NULL AND lease_expiration_timestamp IS NOT NULL)
        OR status != 'running'
    ),
...
CREATE INDEX IF NOT EXISTS jobs_queued_idx
    ON jobs(submitted_at_ms, id)
    WHERE status = 'queued';

CREATE INDEX IF NOT EXISTS jobs_expired_running_idx
    ON jobs(lease_expiration_timestamp, submitted_at_ms, id)
