...
async fn jobs_can_be_filtered_by_creator_and_creation_timestamp() {
    ...
    let store = open_store_with_clock(Arc::new(TestClock::new(100))).await;
...
async fn claiming_a_job_sets_its_status_to_running() {
    ...
    let store = open_store_with_clock(clock).await;
    store
        .create_job(
            new_job(
                "test-user",
...
async fn updating_progress_keeps_job_running() {
    let clock = Arc::new(TestClock::new(10));
    let store = open_store_with_clock(clock.clone()).await;
    let job = new_job(
        "test-user",
        "/datasets/source",
        "s3://destination-bucket/data.lance",
